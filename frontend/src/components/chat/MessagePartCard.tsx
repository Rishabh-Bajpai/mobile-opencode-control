      ...
      <div className="tool-input-params">
        {inputKeys.map((k) => (
          <div key={k} className="tool-input-param">
            ...
            <span className="tool-param-val">{String(input[k]).slice(0, 200)}</span>
          </div>
        ))}
      </div>
    );
  ...
  if (state.output) {
    ...
    outputNode = (
      <div className="tool-output">
        ...
        <pre className="tool-output-content">{state.output.length > 5000 ? state.output.slice(0, 5000) + "\n… (truncated)" : state.output}</pre>
      </div>
    );
  }
